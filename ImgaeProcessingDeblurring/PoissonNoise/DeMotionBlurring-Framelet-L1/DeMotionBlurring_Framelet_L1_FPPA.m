...
%    minimize_u  varphi_x(Ku) + lambda || Du ||_1
...
peakval = paraModel.PoissonNoiseScale;  % peakval affects the computation of psnr
                                        % the current setting is for Possion noise
                                        % if for Gaussian noise, set peakval=255
...
NumShow = paraAlgo.Out.NumShow;  %% number of iterations showing progress
%% initial value
[N1,N2] = size(Img_ref);
u1 = zeros(N1,N2);  
...
for k = 1:MaxIter
    ...
    % recording 
    History.PSNR(k) = psnr(u2,Img_ref,peakval);
