...
%                     the computation of prox_{varphi_x \circ K} needs an inverse of a matrix and for this case of blurring,
%                     there is no fast algorthm(not like padding with periodic condition) provided, thus inner iteration
%                     for estimating prox_{varphi_x \circ K} is involved
%
%               D is (anisotrophic) first order differnce matrix
%
...
paraAlgo.Out.rho = 0.3;
paraAlgo.Out.beta = (1+1e-6)*8/paraAlgo.Out.rho;   %% convergence condition: beta*rho>||first order difference matrix||_2^2=1
...
%%  Training
tic
[u, ~,~, History] = DeMotionBlurring_ATV_L1_FPPA(paraAlgo, paraModel, Img_obs, Img_ref);
time = toc;
fname = sprintf('DeMotionBlurring_ATV_PSNR%2.5f_lambda%2.2f_OUTrho%2.2f_INrho%2.2f.mat',...
    History.PSNR(end), paraModel.lambda, paraAlgo.Out.rho, paraAlgo.In.rho)
save(fname, 'u', 'History', 'paraModel', 'paraAlgo')
