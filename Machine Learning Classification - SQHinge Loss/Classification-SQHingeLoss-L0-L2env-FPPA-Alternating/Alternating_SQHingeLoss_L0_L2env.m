function [u1, v1, w1, History] = Alternating_SQHingeLoss_L0_L2env(KernelMat, PredMat, LabelTrain, LabelTest, paraAlgo, paraModel)
...
for k = 1:1:MaxIter
    ...
    InnerTOL = InnerSTOP * norm(u2-u1)^2/2 + TargetFunction(u2,v1,paraModel);
    [v2, w2, InnerNum, TargetValue] = InnerIteration_FPPA_psi_B(theta*gamma/lambda, theta*u2+(1-theta)*v1, u2, v1, w1, paraAlgo, paraModel, InnerTOL, k);
%     [v2, w2, InnerNum, TargetValue] = InnerIteration_FPPA_psi_B_1stepInnerIteration(theta*gamma/lambda, theta*u2+(1-theta)*v1, u2, v1, w1, paraAlgo, paraModel, InnerTOL, k);
    %% Record
    ...
    History.AccuracyTrain(k) = ShowAccuracyClassification(sign(KernelMat*u2), LabelTrain);      
    History.AccuracyTest(k) =  ShowAccuracyClassification(sign(PredMat*u2), LabelTest);
   
    History.u_RelDiff(k) = norm(u2-u1)/norm(u2);
    History.v_RelDiff(k) = norm(v2-v1)/norm(v2);
    %% Show Progress
    if rem(k,NumShow)==0
        fprintf('Iter: %d, TargetValue: %f; NNZ: %d; TRain: %0.2f%%; TEst: %0.2f%%; u_RelDiff: %0.6f; v_RelDiff: %0.6f; InnerIteration:%d\n',...
            k, History.TargetValue(k), History.NNZ(k), History.AccuracyTrain(k), History.AccuracyTest(k), History.u_RelDiff(k), History.v_RelDiff(k), sum(History.InnerNum(end-NumShow+1:end)))
    end
