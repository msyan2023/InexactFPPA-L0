...
function [GaussMat, DistanceMat] = GaussKernel(TrainPoints,TestPoints, sigma)
...
GaussMat = exp(-DistanceMat/(2*sigma^2));

assert (size(GaussMat,1) == size(TestPoints,2))

assert (size(GaussMat,2) == size(TrainPoints,2))
